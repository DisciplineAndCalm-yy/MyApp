  ...
  <el-form ref="form" :rules="rules2" :model="form" label-width="100px">
    ...
    <table class="table">
      ...
      <tr v-for="(v,i) in form.questionData" :key="i">
        ...
        <td>
          <el-input-number v-model="v.style.top" :precision="3" :step="0.001"></el-input-number>
        </td>
        <td>
          <el-input-number v-model="v.style.left" :precision="3" :step="0.001"></el-input-number>
        </td>
      </tr>
    </table>
...
<script>
import { asyncuploadQuestion } from "../../../plugins/api.js";
  ...
  methods: {
    ...
    async onSubmit() {
      this.$refs["form"].validate(async valid => {
        if (valid) {
          try {
            // let questionData = JSON.stringify(this.form.questionData);
            let temp = JSON.stringify(this.form);
            let tmp = JSON.parse(temp);
            //tmp.questionData = questionData;
            tmp.video_ID = this.lid;
            await asyncuploadQuestion(tmp);
            this.start();
            this.$message({
              message: "恭喜你，添加成功",
              type: "success"
